...
def get_domain(domain_name, args, img_size=None):
    try:
        domain = registries.get_domain(domain_name)(args, img_size)
    except KeyError:
...
def get_trainer(
    ...
    loggers = get_loggers(
        name, version, args.loggers, model, args, tags, source_files
    )
    ...
    if len(loggers):
        ...
        if early_stopping_patience is not None:
            callbacks.append(
                EarlyStopping(
                    monitor=monitor_loss, patience=early_stopping_patience
                )
            )
    ...
    _trainer_args = {
        ...
        "logger": loggers,
        "callbacks": callbacks,
        "resume_from_checkpoint": get_checkpoint_path(
