...
def save_labels(path_root, dataset):
    classes, locations, radii = dataset["classes"], dataset["locations"], dataset["sizes"]
    rotations, colors = dataset["rotations"], dataset["colors"]
    header = ["class", "x", "y", "scale", "rotation", "r", "g", "b"]
    with open(path_root, "w") as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for cls, location, scale, rotation, color in zip(classes, locations, radii, rotations, colors):
...
def main():
    seed = 0
    image_size = 32
    dataset_location = Path("/mnt/SSD/datasets/shapes_v9")
